...
def current_datetime(request):
    now = datetime.datetime.now()notatag
...
```
>>> from django.template import Template
>>> t = Template('``{\% n o t a t a g \%}')``
Traceback (most recent call last):
  File "<stdin>", line 1, in ?
  ...
django.template.TemplateSyntaxError: Invalid block t a g: ``'n o t a tag'``
```

这里，块标签(block tag)指向的是`` {\% n o ta ta g \%}``，块标签与模板标签是同义的。
